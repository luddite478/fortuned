...
/*
   sv_rewind() - change playback position WITH stop/restart (will cause audio cut)
   For seamless position changes during playback, use sv_set_position() instead.
*/
...
/*
   CUSTOM ADDITION: sv_pattern_set_flags() - set pattern flags;
   flags: bit mask of pattern flags to set/clear
     SUNVOX_PATTERN_FLAG_NO_NOTES_OFF (1<<1) - don't send note-off at pattern end (for seamless looping)
   set: 1 to set flags, 0 to clear flags
   return value: 0 (success) or -1 (error);
*/
int sv_pattern_set_flags( int slot, int pat_num, uint32_t flags, int set ) SUNVOX_FN_ATTR; /* USE LOCK/UNLOCK! */

#define SV_PATTERN_FLAG_NO_NOTES_OFF  (1<<1)

/*
   CUSTOM ADDITION: sv_enable_supertracks() - enable SunVox 2.0 supertracks mode;
   This mode is required for pattern flags like NO_NOTES_OFF to work properly.
   enable: 1 to enable supertracks, 0 to disable;
   return value: 0 (success) or -1 (error);
*/
int sv_enable_supertracks( int slot, int enable ) SUNVOX_FN_ATTR; /* USE LOCK/UNLOCK! */
