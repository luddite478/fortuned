...
// ===== FORTUNED MODIFICATION: Platform-specific logging =====
    ...
    #define SVLIB_LOG(fmt, ...) __android_log_print(ANDROID_LOG_INFO, "SUNVOX_LIB", fmt, ##__VA_ARGS__)
#else
    #include <stdio.h>
    #define SVLIB_LOG(fmt, ...) printf("SUNVOX_LIB: " fmt "\n", ##__VA_ARGS__)
#endif
// ===== END MODIFICATION =====
...
SUNVOX_EXPORT int sv_set_pattern_loop( int slot, int pattern_num )
{
    ...
    SVLIB_LOG("🔧 [sv_set_pattern_loop] CALLED with pattern_num=%d (BEFORE: single_pattern=%d, next_single=%d)", 
              pattern_num, s->single_pattern_play, s->next_single_pattern_play);
    ...
    if( pattern_num >= 0 )
    {
        ...
        {
            SVLIB_LOG("❌ [sv_set_pattern_loop] INVALID pattern %d", pattern_num);
    ...
    SVLIB_LOG("✅ [sv_set_pattern_loop] SET single_pattern=%d, next_single=%d", 
...
SUNVOX_EXPORT int sv_set_pattern_sequence( int slot, int* pattern_ids, int count )
{
    ...
    // Validate count
    if( count < 0 || count > SUNVOX_MAX_PATTERN_SEQUENCE ) return -1;
    
    // Validate all pattern IDs
    for( int i = 0; i < count; i++ )
    {
        int pat_num = pattern_ids[ i ];
        if( pat_num < 0 || pat_num >= SUNVOX_MAX_PATTERN_LOOP_TRACKING ) return -1;
        if( (unsigned)pat_num >= (unsigned)s->pats_num || !s->pats[ pat_num ] )
            return -1;
    }
...
SUNVOX_EXPORT int sv_pattern_set_flags( int slot, int pat_num, uint32_t flags, int set )
{
    ...
    sunvox_change_pattern_flags( pat_num, flags, 0, set != 0, s );
    return 0;
}
#ifdef OS_ANDROID
SUNVOX_EXPORT JNIEXPORT jint JNICALL Java_nightradio_sunvoxlib_SunVoxLib_pattern_1set_1flags( JNIEnv* je, jclass jc, jint slot, jint pat_num, jint flags, jint set )
{
    return sv_pattern_set_flags( slot, pat_num, (uint32_t)flags, set );
}
#endif
...
#ifdef OS_ANDROID
SUNVOX_EXPORT JNIEXPORT jint JNICALL Java_nightradio_sunvoxlib_SunVoxLib_enable_1supertracks( JNIEnv* je, jclass jc, jint slot, jint enable )
{
    return sv_enable_supertracks( slot, enable );
}
#endif
