...
class SequencerState extends ChangeNotifier {
  ...
  int get numSoundGrids => _soundGridOrder.length;
  ...
  void startSequencer() {
    ...
    final isSong = _sectionPlaybackMode == SectionPlaybackMode.song;
    _sequencerLibrary.setSongMode(isSong);
    
    // Configure steps length and playback region according to mode
    // Steps length equals current UI grid rows times number of sections in song mode,
    // or just current grid rows in loop mode (we loop a window of size gridRows).
    final totalSteps = isSong ? (_numSections * _gridRows) : _gridRows;
    _sequencerLibrary.setStepsLen(totalSteps);
    
    if (isSong) {
      // Full table region
      _sequencerLibrary.setPlaybackRegionBounds(0, totalSteps);
    } else {
      // Loop current section window
      final sectionStart = _currentSectionIndex * _gridRows;
      _sequencerLibrary.setPlaybackRegionBounds(sectionStart, sectionStart + _gridRows);
    }
    ...
    final int startAbsoluteStep = (_currentSectionIndex * _gridRows);
    final int stepsForStart = isSong ? (_numSections * _gridRows) : _gridRows;
    bool success = _sequencerLibrary.startSequencer(_bpm, stepsForStart, startAbsoluteStep: startAbsoluteStep);
    if (success) {
  ...
  void syncFlutterSequencerGridToNativeSequencerGrid() {
    ...
    // Transfer ALL SECTIONS for ALL sound grids to native table from single source of truth
    int totalCellsSet = 0;
    for (int sectionIndex = 0; sectionIndex < _numSections; sectionIndex++) {
      final List<List<int?>> sectionGrids = _sectionGridData[sectionIndex] ?? List.generate(
        _soundGridSamples.length,
        (i) => List.filled(_gridColumns * _gridRows, null),
      );

      for (int gridIndex = 0; gridIndex < sectionGrids.length; gridIndex++) {
        final gridSamples = sectionGrids[gridIndex];
        int gridCellsSet = 0;
  ...
  void _startUIUpdateTimer() {
    ...
    _sequencerTimer = Timer.periodic(Duration(milliseconds: uiUpdateIntervalMs), (timer) {
      if (!_sequencerLibrary.isSequencerPlaying) {
        // Ensure full stop path runs to clear highlights and reset UI state
        timer.cancel();
        _sequencerTimer = null;
        stopSequencer();
        return;
      ...
      if (_sectionPlaybackMode == SectionPlaybackMode.song) {
        ...
        // Update UI section if it changed: ensure we save/load grids for correct UI state
        if (currentSectionFromAbsoluteStep != _currentSectionIndex) {
          // Use existing switching logic which saves outgoing section and loads incoming section data.
          // This is UI-only while playing; native playback remains uninterrupted.
          _switchToSection(currentSectionFromAbsoluteStep);
        }
...
''';
  ...
  void initializeSoundGrids(int numGrids) {
    ...
    // Ensure backing stores exist and have the correct number of grids for ALL sections
    for (int s = 0; s < _numSections; s++) {
      final sectionGrids = _sectionGridData[s] ?? [];
      // Grow grids to numGrids
      while (sectionGrids.length < numGrids) {
        sectionGrids.add(List.filled(_gridColumns * _gridRows, null));
      }
      // Shrink grids if too many
      while (sectionGrids.length > numGrids) {
        sectionGrids.removeLast();
      }
      _sectionGridData[s] = sectionGrids;
    }

    // Bind current section's data to the working grid by reference (single source of truth)
    _soundGridSamples = _sectionGridData[_currentSectionIndex]!;
  ...
  void addSoundGrid() {
    ...
    // Add new empty grid to ALL sections
    for (int s = 0; s < _numSections; s++) {
      final grids = _sectionGridData[s] ?? [];
      grids.add(List.filled(_gridColumns * _gridRows, null));
      _sectionGridData[s] = grids;
    }
    // Rebind working view for current section
    _soundGridSamples = _sectionGridData[_currentSectionIndex]!;
  ...
  void removeSoundGrid() {
    ...
    // Remove the last grid from ALL sections
    for (int s = 0; s < _numSections; s++) {
      final grids = _sectionGridData[s];
  ...
  void increaseGridRows() {
    ...
    // 🎯 REAL-TIME UPDATE: If sequencer is playing, update native logical length and region
    if (_isSequencerPlaying) {
      final isSong = _sectionPlaybackMode == SectionPlaybackMode.song;
      final totalSteps = isSong ? (_numSections * _gridRows) : _gridRows;
      _sequencerLibrary.setStepsLen(totalSteps);
      ...
      debugPrint('🔄 Updated playing steps/region after resize: rows ${oldRows} → $_gridRows');
    }
  ...
  void _resizeAllSoundGrids(int oldRows, int newRows) {
    ...
    for (int s = 0; s < _numSections; s++) {
      final sectionGrids = _sectionGridData[s]!;
      for (int gridIndex = 0; gridIndex < sectionGrids.length; gridIndex++) {
        final oldGrid = sectionGrids[gridIndex];
        final newGrid = <int?>[];
      ...
      if (newSize > oldSize) {
        ...
        sectionGrids[gridIndex] = newGrid;
      }
    }
    // Rebind current section
    _soundGridSamples = _sectionGridData[_currentSectionIndex]!;
  }
  ...
  String _serializeCurrentState() {
    final state = {
      ...
      // Persist all sections' grids
      'sectionGridData': _sectionGridData.map((k, v) => MapEntry(k.toString(), v)),
  ...
  Future<void> _deserializeAndApplyState(String stateJson) async {
    try {
      ...
      _numSections = state['numSections'] ?? 1;
      ...
      // Apply section grid data for ALL sections
      final sectionGridData = state['sectionGridData'] as Map<String, dynamic>?;
      final restoredSectionIndex = state['currentSectionIndex'] ?? 0;
      if (sectionGridData != null) {
        _sectionGridData.clear();
        sectionGridData.forEach((key, value) {
          final sectionIndex = int.tryParse(key);
          if (sectionIndex != null) {
            final grids = (value as List<dynamic>)
                .map((grid) => (grid as List<dynamic>).cast<int?>())
                .toList();
            _sectionGridData[sectionIndex] = grids;
          }
        });
      } else {
        // Fallback: ensure at least current section exists
        _sectionGridData[_currentSectionIndex] = _sectionGridData[_currentSectionIndex] ??
            List.generate(_soundGridOrder.isNotEmpty ? _soundGridOrder.length : 1, (i) => List.filled(_gridColumns * _gridRows, null));
      }

      // Rebind working view to restored current section
      _currentSectionIndex = (restoredSectionIndex is int) ? restoredSectionIndex : 0;
      _soundGridSamples = _sectionGridData[_currentSectionIndex] ??
          List.generate(_soundGridOrder.isNotEmpty ? _soundGridOrder.length : 1, (i) => List.filled(_gridColumns * _gridRows, null));
  ...
  Future<void> resetToFreshState() async {
    ...
    // Sections: reset to a single blank section (single source of truth)
    _numSections = 1;
    _currentSectionIndex = 0;
    _sectionLoopCounts = [1];
    _sectionGridData.clear();
    _sectionGridData[_currentSectionIndex] = List.generate(
      1, // will be re-bound after addSoundGrid
      (index) => List.filled(_gridColumns * _gridRows, null),
    );
    _soundGridSamples = _sectionGridData[_currentSectionIndex]!;
    // Keep UI sectioning only in Flutter; native uses playback region now
  ...
  List<int?> getSectionGridSamples(int sectionIndex, {int gridIndex = 0}) {
    ...
    if (sectionData != null && gridIndex < sectionData.length) {
      return List<int?>.from(sectionData[gridIndex]);
    }
    return List.filled(_gridRows * _gridColumns, null);
  }

  // Calculate total steps in the native table (sum of all section sizes)
  int _calculateTotalTableSteps() {
    // Currently all sections use the same size (_gridRows), but this method
    // makes it clear that we're calculating the total table size for native
    return _numSections * _gridRows;
  }
  ...
  // Core section switching logic (UI-only while playing)
  void _switchToSection(int newSectionIndex) {
    if (newSectionIndex < 0 || newSectionIndex >= _numSections) return;
    
    // 1. Update current section index
    _currentSectionIndex = newSectionIndex;
    
    // 2. Rebind working view by reference (backing stores are created at init/creation time)
    _soundGridSamples = _sectionGridData[_currentSectionIndex]!;
    
    // 4. Reset UI step indicator
    if (!_isSequencerPlaying) {
      _currentStep = -1;
      _currentStepNotifier.value = -1;
    }
    ...
    // Update native playback region in loop mode so wrap window follows UI
    if (_isSequencerPlaying && _sectionPlaybackMode == SectionPlaybackMode.loop) {
      final sectionStart = _currentSectionIndex * _gridRows;
      _sequencerLibrary.setPlaybackRegionBounds(sectionStart, sectionStart + _gridRows);
    }

         // 6. UI: Interface refreshes to show new section's content
    notifyListeners();
  ...
  void createEmptySection() {
    ...
    // Update native table size (native doesn't have sections, just total steps)
    _sequencerLibrary.setStepsLen(_calculateTotalTableSteps());
    
    // Ensure backing store exists for the new section
    final numGrids = _soundGridSamples.length;
    _sectionGridData[newSectionIndex] = List.generate(
      numGrids,
      (index) => List.filled(_gridColumns * _gridRows, null),
    );
  ...
  void createSectionCopyFrom(int sourceSectionIndex) {
    ...
    if (sourceSectionIndex == _currentSectionIndex) {
      // Copy current visible data by value
    ...
    // Store copied data for new section (new backing store)
