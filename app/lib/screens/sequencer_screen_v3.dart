...
class SequencerScreenV3 extends StatefulWidget {
  ...
  @override
  State<SequencerScreenV3> createState() => _SequencerScreenV3State();
}

class _SequencerScreenV3State extends State<SequencerScreenV3> with WidgetsBindingObserver {
  ...
  void _setupThreadsServiceListeners() async {
    ...
    debugPrint('📡 Using global ThreadsService connection in sequencer V3');
  }
  ...
  void _ensureActiveThread() async {
    ...
    if (threadsState.activeThread == null && !sequencerState.isCollaborating) {
      try {
        debugPrint('📝 No active thread found, creating new unpublished thread for sequencer V3');
